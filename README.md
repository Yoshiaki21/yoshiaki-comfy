...
`Select to add LoRA`にはファイル名だけ（フォルダ名なし）が表示され、クリックするとフォルダ絞り込み済みのポップアップメニューから選べます。内部的には、ComfyUIの「モデルファイル存在チェック」と一致させる必要があるフォルダ込みのフルパスを保持する本体のウィジェットを非表示にし、見た目上の`Select to add LoRA`はボタン＋独自ポップアップの別ウィジェットに差し替えることで、余計な行を増やさずに警告を回避しています。
...
- **Yoshiaki LoRA Caption Load**: `path`で指定したフォルダ内のPNG画像を全て読み込み、`Image list`（画像バッチ）と`Name list`（ファイル名一覧、改行区切り）、`path`（そのまま）を出力する。`Name list`と`Image list`は同じ1回のフォルダ走査結果を**ファイル名順にソート**したもので、必ず同じ件数・同じ順序になる（拡張子の大文字小文字は区別しない）
- **Yoshiaki LoRA Caption Save**: `Name list`・`path`・`text`（キャプション文字列）を受け取り、画像ファイルと同じ名前の`.txt`を保存する。`text`にリスト出力（WD14 Tagger / LLMCaptionGenerator）を繋ぐと全件をまとめて受け取り、**`Name list`のi番目とキャプションのi番目を常に1対1で対応付けて**書き出す。`prefix`（任意）を指定すると、トリガーワード等をキャプションの先頭に自動で付加できる
  ...
  - `overwrite`（任意、既定OFF）: 対応付け自体はON/OFFに関係なく常に位置ベースの1対1。OFFのときは保存先に既に`.txt`がある名前を**スキップ**する（既存ファイルには一切触れない）。ONにすると既存ファイルの有無を無視して`.txt`と画像コピーを上書きする。同じモデルの比較のため、出力先を変えずに何度も captioning をやり直したい場合などに使う
  - `Name list`の件数とキャプションの件数が食い違う場合は警告を出し、少ない方の件数分だけ書き出す（余ったキャプションが別のファイル名に回ることはない）
...
- 画像ファイル名とキャプション`.txt`の中身がずれる不具合を修正（2026-09-08）。原因は2つ: ①`Yoshiaki LoRA Caption Save`が`overwrite=ON`時にノードインスタンス内のカウンターで「今回は何番目か」を数えていたが、ComfyUIはキュー実行をまたいでノードインスタンスを再利用するため、途中キャンセルや画像枚数の違うフォルダへの切り替えでカウンターがずれ続けていた。②`Yoshiaki LoRA Caption Load`が`Name list`を`glob('*.png')`、`Image list`を`os.listdir`＋小文字比較という別々の走査で作っていたため、Linuxで`.PNG`などの大文字拡張子やドット始まりのファイルが片方にだけ入り件数が食い違っていた。Saveは`INPUT_IS_LIST`で全キャプションをまとめて受け取る位置ベースの1対1対応に、Loadは単一走査＋ソートに変更した
